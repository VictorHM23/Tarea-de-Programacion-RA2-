...
public:
    ...
    {
        cout << "Esperando a conexiones de clientes\n";
        while (true) {
            ...
            *clientSocket = accept(serverSocket, nullptr, nullptr);
            ...
            cout << "Se inicio una nueva partida [" << ip << ":" << port << "]" << endl;
            pthread_t thread_id;
    ...
    static void* jugar(void* arg) {
        ...
        while (true) {
            ...
            if (!juego.realizarJugada(COLUMNA, 'C')) {
                ss << "Columna no valida, intentalo de nuevo, recuerde que son numeros entre (1-7).\n";
            } else {
                ss << "Cuatro en linea [" << ip << ":" << port << "]: El cliente jugó en la columna " << COLUMNA << ".\n";
                ss << juego.BOARDToString();
                cout << "Cuatro en linea [" << ip << ":" << port << "]: El cliente jugó en la columna " << COLUMNA << ".\n";

                if (juego.verificarGanador('C')) {
                    ss << "\n >>> El cliente ha ganado la partida.\n";
                    cout << "Cuatro en linea [" << ip << ":" << port << "]: El cliente ha ganado la partida. Fin del juego.\n";
                ...
                if (juego.juegaServidor(COLUMNA_JUGADA)) {
                    ss << "Cuatro en linea [" << ip << ":" << port << "]: El servidor jugó en la columna " << COLUMNA_JUGADA << ".\n";
                    ss << juego.BOARDToString();
                    cout << "Cuatro en linea [" << ip << ":" << port << "]: El servidor jugó en la columna " << COLUMNA_JUGADA << ".\n";

                    if (juego.verificarGanador('S')) {
                        ss << "\n El servidor ha ganado la partida.\n";
                        cout << "Cuatro en linea [" << ip << ":" << port << "]: El servidor ha ganado la partida. Fin del juego.\n";
                        send(clientSocket, ss.str().c_str(), ss.str().length(), 0);
                        break;
                    }
                }
